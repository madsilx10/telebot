...
async def click_btn(app, bot_username, msg, index, label=""):
    """Klik button ReplyKeyboard atau InlineKeyboard."""
    if not msg or not msg.reply_markup:
        return None

    # ReplyKeyboardMarkup — kirim teks buttonnya
    if hasattr(msg.reply_markup, 'keyboard'):
        all_btns = [btn for row in msg.reply_markup.keyboard for btn in row]
        if not all_btns:
            return None
        # Button bisa string atau objek
        get_text = lambda b: b if isinstance(b, str) else b.text
        print(f"[Akun {index}] {label} Buttons: {[get_text(b) for b in all_btns]}")
        for kw in SUBMIT_KEYWORDS:
            for btn in all_btns:
                if kw in get_text(btn).lower():
                    await app.send_message(bot_username, get_text(btn))
                    print(f"[Akun {index}] ✅ Klik: '{get_text(btn)}'")
                    return get_text(btn)
        await app.send_message(bot_username, get_text(all_btns[0]))
        print(f"[Akun {index}] ✅ Klik fallback: '{get_text(all_btns[0])}'")
        return get_text(all_btns[0])
...
async def run_account(session_string, twitter_username, wallet_address, email, index, bot_username, start_param, do_chat):
    ...
    async with Client(
        ...
        msg = await get_latest_msg(app, bot_username)
        clicked = await click_btn(app, bot_username, msg, index, "[Submit Details]")
        if clicked:
