...
async def run_account(session_string, link_x, email, wallet, index, bot_username, start_param, do_chat):
    ...
    async with Client(
        ...
        clicked = await click_inline(msg, ['submit', 'detail', 'start', 'begin', 'continue'])
        if clicked:
        ...
        # STEP 5: Muncul pesan follow X → klik Done (inline)
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")

        clicked = await click_inline(msg, ['done', 'selesai', 'ok'])
        if clicked:
            print(f"[Akun {index}] ✅ Klik Done: {clicked}")
        await asyncio.sleep(3)

        # STEP 6: Muncul pesan → klik Yes (inline)
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")

        clicked = await click_inline(msg, ['yes', 'ya', 'iya', 'confirm'])
        if clicked:
        ...
        # STEP 9: Klik Continue (inline)
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")

        clicked = await click_inline(msg, ['continue', 'lanjut', 'next'])
        if clicked:
            print(f"[Akun {index}] ✅ Klik Continue: {clicked}")
        await asyncio.sleep(3)

        # STEP 10: Klik Complete the airdrop (inline)
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")

        clicked = await click_inline(msg, ['complete', 'finish', 'claim', 'airdrop'])
        if clicked:
