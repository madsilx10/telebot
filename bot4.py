...
async def click_reply_btn(app, bot_username, keywords):
    """Klik tombol reply keyboard"""
    for kw in keywords:
        await app.send_message(bot_username, kw)
        return kw
    return None
...
async def run_account(session_string, link_x, email, wallet, index, bot_username, start_param, do_chat):
    ...
    async with Client(
        ...
        # STEP 3: Klik tombol "Submit your details"
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")
        ...
        if msg and msg.reply_markup:
            for row in msg.reply_markup.keyboard:
                for btn in row:
                    if any(k in btn.lower() for k in ['submit', 'detail', 'start', 'begin']):
                        await app.send_message(bot_username, btn)
                        print(f"[Akun {index}] ✅ Klik: {btn}")
        ...
        # STEP 5: Muncul pesan follow X → klik Done
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")
        ...
        if msg and msg.reply_markup:
            for row in msg.reply_markup.keyboard:
                for btn in row:
                    if any(k in btn.lower() for k in ['done', 'selesai']):
                        await app.send_message(bot_username, btn)
                        print(f"[Akun {index}] ✅ Klik: {btn}")
                        await asyncio.sleep(3)
                        break

        # STEP 6: Muncul pesan → klik Yes
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")
        ...
        if msg and msg.reply_markup:
            for row in msg.reply_markup.keyboard:
                for btn in row:
                    if any(k in btn.lower() for k in ['yes', 'ya', 'iya']):
                        await app.send_message(bot_username, btn)
                        print(f"[Akun {index}] ✅ Klik: {btn}")
                        await asyncio.sleep(3)
                        break
        ...
        if msg and msg.reply_markup:
            for row in msg.reply_markup.keyboard:
                for btn in row:
                    if any(k in btn.lower() for k in ['continue', 'lanjut', 'next']):
                        await app.send_message(bot_username, btn)
                        print(f"[Akun {index}] ✅ Klik: {btn}")
                        await asyncio.sleep(3)
                        break

        # STEP 10: Klik Complete the airdrop
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")
        ...
        if msg and msg.reply_markup:
            for row in msg.reply_markup.keyboard:
                for btn in row:
                    if any(k in btn.lower() for k in ['complete', 'finish', 'claim', 'airdrop']):
                        await app.send_message(bot_username, btn)
                        print(f"[Akun {index}] ✅ Klik: {btn}")
                        await asyncio.sleep(3)
                        break
