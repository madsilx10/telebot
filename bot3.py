...
async def run_account(session_string, index, bot_link, bot_username, x_profiles, quotes, posts, emails, wallets, mapping):
    ...
    async with Client(
        ...
        # Baca soal (gambar teks) lalu pilih emoji di grid
        try:
            soal_text = ""
            grid_msg  = None
            ...
            async for msg in app.get_chat_history(bot_username, limit=10):
                if msg.photo:
                    ...
                    if msg.reply_markup and hasattr(msg.reply_markup, 'inline_keyboard'):
                        # ini pesan grid emoji
                        grid_msg = msg
                    else:
                        # ini pesan soal
                        soal_text = ocr_result
                        print(f"[~] Soal: {soal_text}")

            if soal_text and grid_msg:
                all_btns = [
                ...
                if target_emoji:
                    ...
                    await grid_msg.click(btn_index // cols, btn_index % cols)
