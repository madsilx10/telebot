...
OCR_API_KEY = ""   # isi API key OCR.space
  ...
  └─────────────────────────────┘""")
    ...
    async with Client(
        ...
        try:
            ...
            print(f"         ⚠️ Start gagal: {e}")
            return

        # Klik Join Airdrop Register
        await click_keyboard(app, bot_username, ["join airdrop", "register"], "Join Airdrop")
        ...
        try:
            async for msg in app.get_chat_history(bot_username, limit=5):
                if msg.photo and msg.reply_markup and hasattr(msg.reply_markup, 'inline_keyboard'):
                    ...
                    print(f"         🔍 OCR hasil: {ocr_text if ocr_text else 'tidak terdeteksi'}")
                    ...
                    print(f"         ✅ Pilih gambar: {all_btns[btn_index]}")
                    await asyncio.sleep(2)
                    break
        except Exception as e:
            print(f"         ⚠️ Pilih gambar gagal: {e}")

        # Submit data
        await submit_text(app, bot_username, email,     "Email     ")
        await submit_text(app, bot_username, wallet,    "Wallet    ")
        await submit_text(app, bot_username, x_profile, "X Profile ")
        await submit_text(app, bot_username, quote,     "Quote     ")
        await submit_text(app, bot_username, post,      "Post      ")
...
async def main():
  ...
  ╠══════════════════════════════════╣
  ║   Total Akun  : {total:<17}║
  ║   X Profile   : {len(x_profiles):<17}║
  ║   Quote       : {len(quotes):<17}║
  ║   Post        : {len(posts):<17}║
  ║   Email       : {len(emails):<17}║
  ║   Wallet      : {len(wallets):<17}║
  ╠══════════════════════════════════╣
  ║   1. Semua akun                  ║
  ║   2. Pilih satu akun             ║
  ║   3. From akun ke-N              ║
  ╚══════════════════════════════════╝""")
    ...
    for i in indices:
        ...
        if i != indices[-1]:
            print(f"  ⏳ Delay {DELAY_BETWEEN_ACCOUNTS} detik...\n")
