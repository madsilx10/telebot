...
async def run_account(session_string, link_x, email, wallet, index, bot_username, start_param, do_chat):
    ...
    async with Client(
        ...
        # STEP 9.5: Join semua group yang ada di pesan
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:120]}")

        # Extract semua t.me links
        tme_links = []
        ...
        for link in tme_links:
            try:
                ...
                break  # berhasil join, stop
            except Exception as e:
                if "already" in str(e).lower() or "USER_ALREADY" in str(e):
                    print(f"[Akun {index}] ✅ Sudah join: {group_username}")
                    break  # udah join, stop
                else:
                    print(f"[Akun {index}] ⚠️ Group penuh/gagal, coba berikutnya: {e}")
