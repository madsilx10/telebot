...
async def click_button(client: Client, chat: str, msg: Message, keyword: str) -> bool:
    """Handle ReplyKeyboardMarkup (kirim teks) dan InlineKeyboard (msg.click)."""
    if not msg.reply_markup:
        return False

    markup = msg.reply_markup

    # ReplyKeyboardMarkup → kirim teks tombol
...
async def run_account(session_str: str, x_profile: str, email: str, wallet: str, index: int):
    ...
    async with client:
        ...
        if not last_msg:
            ...
            return
        print(f"{label} Bot: {(last_msg.text or '')[:80]}")
        if not await click_button(client, BOT_USERNAME, last_msg, "complete the airdrop"):
