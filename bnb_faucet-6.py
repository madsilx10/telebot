...
async def tunggu_balas(app, last_id, jumlah=1, timeout=TIMEOUT):
    """Tunggu sejumlah `jumlah` pesan baru dari bot."""
    print(f"[*] Nunggu {jumlah} balas bot...")
    dapat = 0
    current_id = last_id
    ...
    for _ in range(timeout):
        ...
        async for msg in app.get_chat_history(BOT, limit=5):
            if msg.id > current_id and msg.from_user and msg.from_user.is_bot:
                print(f"[*] Bot balas ({dapat+1}): {msg.text[:60] if msg.text else '(non-text)'}")
                current_id = msg.id
                dapat += 1
                break
        if dapat >= jumlah:
...
async def claim(session_string, wallet, akun_ke):
    ...
    async with app:
        ...
        print("[*] /start sent")
        last_id = await tunggu_balas(app, last_id, jumlah=1)

        await app.send_message(BOT, 'faucet')
        print("[*] faucet sent")
        last_id = await tunggu_balas(app, last_id, jumlah=2)  # tunggu 2 pesan: ticket + minta address

        await app.send_message(BOT, wallet)
        print(f"[*] Address sent: {wallet}")
        await tunggu_balas(app, last_id, jumlah=1)
