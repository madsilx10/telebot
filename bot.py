...
def extract_tme_from_entities(msg):
    """Ambil t.me link dari hyperlink entities di pesan"""
    if not msg.entities:
        return None
...
async def click_btn(msg, keywords):
    """Klik tombol inline yang mengandung keyword"""
...
async def run_account(session_string, twitter_username, wallet_address, index):
    ...
    async with Client(
        ...
        # STEP 7: Baca pesan minta username X
        msg = await get_latest_msg(app, BOT_USERNAME)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")
        ...
        # STEP 8: Popup advertiser channel — langsung klik Done (skip join)
        msg = await get_latest_msg(app, BOT_USERNAME)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")
