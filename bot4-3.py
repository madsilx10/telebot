...
async def run_account(session_string, link_x, email, wallet, index, bot_username, start_param, do_chat):
    ...
    async with Client(
        ...
        # STEP 9.5: Join group kalau ada link (sebelum complete)
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        ...
        def _extract_tme(m):
            ...
            if m.entities:
                for entity in m.entities:
                    ...
                    elif entity.type.name == "URL": url = (m.text or "")[entity.offset:entity.offset+entity.length]
                    if url and 't.me/' in url: return url
            t = m.text or ""
            import re as re2
            match = re2.search(r'https?://t\.me/[\w+/]+', t)
            if match: return match.group(0)
            return None

        link = _extract_tme(msg)
        if link:
            try:
