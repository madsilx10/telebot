...
RANDOM_CHATS = [
    "Hi", "hi", "hello", "Hello", "hey", "hi guys", "Hello guys"
]
...
# ── Core logic per akun ──────────────────────────────────
async def run_account(session_string, twitter_username, wallet_address, index, bot_username, start_param, do_chat=True):
    print(f"\n{'='*50}")
    ...
    async with Client(
        ...
        if link:
            try:
                ...
                # Baru random chat kalau do_chat
                if do_chat:
        ...
        # STEP 6.5: Join channel (kalau ada)
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        link = extract_tme_from_entities(msg) or extract_tme_link(text)
        if link:
            try:
                channel_username = link.replace("https://t.me/", "").replace("http://t.me/", "").replace("t.me/", "").strip("/")
                await app.join_chat(channel_username)
                print(f"[Akun {index}] ✅ Join channel: {channel_username}")
                await asyncio.sleep(3)
                clicked = await click_btn(msg, ['done', 'selesai'])
                if clicked:
                    print(f"[Akun {index}] ✅ Klik Done (channel): {clicked}")
                await asyncio.sleep(3)
            except Exception as e:
                print(f"[Akun {index}] ⚠️ Join channel gagal: {e}")
        else:
            print(f"[Akun {index}] Tidak ada task join channel, lanjut...")
...
async def main():
    ...
    for i in indices:
        ...
        if chat_mode == "1":
            ...
            do_chat = False
        await run_account(sessions[i], twitter.strip(), wallet.strip(), i + 1, bot_username, start_param, do_chat)
        if i != indices[-1]:
