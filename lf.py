...
async def run_usn_discord_only(session_name: str, discord_username: str, idx: int):
    ...
    async with Client(
        ...
        try:
            ...
            # ── [1] BUKA MENU (tanpa referral, ga start dari awal) ────────
            print("    [1] Buka bot...")
            ...
            # ── [3] VERIFY (task Discord) ──────────────────────────────────
            print("    [3] Klik Verify (Discord task)...")
            msg = await wait_button(
                client, BOT_USERNAME, "verify",
                after_id=0, timeout=30,
                search_all=True, exclude_ids=set()
            ...
            # ── [4] SUBMIT USN DISCORD ──────────────────────────────────────
            print(f"    [4] Kirim USN Discord: {discord_username}")
            await client.send_message(BOT_USERNAME, discord_username)
            print(f"    🎉 [{idx}] SELESAI!")
...
async def main():
    ...
    print("│  2. Submit USN Discord aja (baru)    │")
    print("└─────────────────────────────────────┘")
