...
# ─────────────────────────────────────────────────────────────────────────────
# MODE BARU: Verify → "Yes, that's me" — pakai sessions.txt, gak perlu /start
# ─────────────────────────────────────────────────────────────────────────────

async def run_verify_me(session_name: str, idx: int):
    print(f"\n{'='*40}")
    print(f"[{idx}] Proses akun (mode: Verify → Yes, that's me)...")
    ...
    async with Client(
        ...
        try:
            ...
            # ── [1] KLIK VERIFY ─────────────────────────────────────────
            # keyword "verify" generic, exclude_ids kosong karena mode ini
            # berdiri sendiri (gak ada verify lain yang udah keklik sebelumnya)
            print("    [1] Klik Verify...")
            msg = await wait_button(
                client, BOT_USERNAME, "verify",
                after_id=0, timeout=30,
            ...
            if msg:
                ...
                print("    ⚠ Tombol Verify tidak ditemukan — skip")
                return

            # ── [2] KLIK "YES, THAT'S ME" ─────────────────────────────────
            # pakai keyword spesifik "that's me" (BUKAN cuma "yes"), karena
            # bot ini juga punya tombol "Yes, that's mine" di task lain —
            # kalau keyword cuma "yes" bisa ketuker/kepencet salah tombol
            #
            # search_all=True WAJIB di sini: bot ini kemungkinan EDIT pesan
            # verify yang sama (msg.id gak berubah) buat nampilin tombol
            # "Yes, that's me", bukan kirim pesan baru. Kalau pakai after_id
            # filter biasa, loop bakal langsung break duluan dan gak pernah
            # nyampe ke pesan itu.
            print("    [2] Klik \"Yes, that's me\"...")
            msg = await wait_button(
                client, BOT_USERNAME, "that's me",
                after_id=after_id, timeout=15,
                search_all=True
            )
            if msg:
                await click_inline(msg, "that's me")
                print(f"    🎉 [{idx}] SELESAI!")
            else:
                print("    ⚠ Tombol 'Yes, that's me' tidak ditemukan")
...
async def main():
    ...
    if mode == "1":
        ...
        queue = pick_accounts(sessions)
        if not queue:
            return

        print(f"\n🚀 Akan proses {len(queue)} akun (mode: Verify → Yes, that's me)...")

        for pos, (idx, sess) in enumerate(queue):
            await run_verify_me(sess, idx)
            if pos < len(queue) - 1:
                gap = round(random.uniform(15, 30), 1)
                print(f"\n⏳ Jeda antar akun: {gap}s...")
                await asyncio.sleep(gap)
