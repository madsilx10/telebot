...
def x_get_user_id(username, auth_token, ct0):
    url = "https://api.twitter.com/1.1/users/show.json"
    r = requests.get(url, headers=x_headers(auth_token, ct0),
                      params={"screen_name": username}, timeout=20)
    r.raise_for_status()
    return r.json()["id_str"]


def x_follow(target_id, auth_token, ct0):
...
def run_follow_x(auth_token, ct0, target_id, idx):
    print(f"\n{'='*40}")
    print(f"[{idx}] Follow @{TARGET_X_USERNAME}...")
    try:
        status, text = x_follow(target_id, auth_token, ct0)
        if status == 200:
            print("    ✅ Follow sukses")
...
async def mode_follow_x():
    ...
    print(f"\n🔎 Ambil user ID @{TARGET_X_USERNAME}...")
    try:
        target_id = x_get_user_id(TARGET_X_USERNAME, *akun_pairs[0])
        print(f"    target_id = {target_id}")
    except Exception as e:
        print(f"❌ Gagal ambil user id target: {e}")
        return
    ...
    for pos, idx in enumerate(indices):
        ...
        run_follow_x(auth_token, ct0, target_id, idx)
