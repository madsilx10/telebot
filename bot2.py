        ...
        try:
            async for msg in app.get_chat_history(group_username, limit=5):
                ...
                if f"@{my_username}".lower() in text.lower():
                    import re as re2
                    match = re2.search(r'[\(\[]?(\d+)\s*([+\-x*/×÷])\s*(\d+)[\)\]]?', text)
                    if match:
                        ...
                        print(f"[Akun {index}] ✅ Jawab captcha group: {ans}")
                        return
...
async def run_account(session_string, twitter_username, wallet_address, email, index, bot_username, start_param, do_chat):
    ...
    async with Client(
        ...
        await app.send_message(bot_username, f"@{my_username}")
        print(f"[Akun {index}] ✅ Kirim usn Tele: @{my_username}")
        await asyncio.sleep(3)
        ...
        await app.send_message(bot_username, rt_link)
        print(f"[Akun {index}] ✅ Kirim RT: {rt_link}")
        await asyncio.sleep(3)
