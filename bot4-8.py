    ...
    async with Client(
        ...
        for link in tme_links:
            try:
                ...
                print(f"[Akun {index}] ❌ Error join {ch_username}: {type(e).__name__}: {e}")
                if "already" in str(e).lower() or "USER_ALREADY" in str(e):
                    print(f"[Akun {index}] ✅ Sudah join channel")
                    break
            await asyncio.sleep(2)
        await asyncio.sleep(10)  # tunggu bot verify join
        ...
        # STEP 10: Join group sebelum Complete
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan group: {text[:120]}")
        ...
        if msg and msg.entities:
            for entity in msg.entities:
                ...
                if url and 't.me/' in url:
                    tme_links_group.append(url)
        if not tme_links_group:
            tme_links_group = re.findall(r'https?://t\.me/[\w+/]+', text)

        print(f"[Akun {index}] Links group: {tme_links_group}")

        for link in tme_links_group:
            try:
                g_username = link.replace("https://t.me/", "").replace("http://t.me/", "").replace("t.me/", "").strip("/")
                await app.join_chat(g_username)
                print(f"[Akun {index}] ✅ Join group: {g_username}")
                break
            except Exception as e:
                print(f"[Akun {index}] ❌ Error join group {g_username}: {type(e).__name__}: {e}")
                if "already" in str(e).lower() or "USER_ALREADY" in str(e):
                    print(f"[Akun {index}] ✅ Sudah join group")
                    break
            await asyncio.sleep(2)
        await asyncio.sleep(3)

        # STEP 11: Klik Complete the airdrop (reply keyboard)
        msg = await get_latest_msg(app, bot_username)
        text = msg.text or msg.caption or "" if msg else ""
        print(f"[Akun {index}] Pesan: {text[:80]}")
