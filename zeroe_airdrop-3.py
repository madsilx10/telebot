...
def clean_channel_link(ch):
    """Bersihin link channel dari trailing path (message id) atau prefix yang gak perlu.
    Contoh: https://t.me/zeroe_official/1 -> https://t.me/zeroe_official
    """
    ch = ch.strip()
    if "t.me/" in ch:
        ch = ch.split("t.me/", 1)[1]
    ch = ch.lstrip("@")
    ch = ch.split("/")[0]  # buang trailing /1, /123, dll
    ch = ch.split("?")[0]  # buang query param kalo ada
    return ch


async def join_channels(app, channels, tag):
    """Join semua channel/group yang ada di channels.txt sebelum mulai flow airdrop."""
    for ch in channels:
        ch = ch.strip()
        if not ch:
            continue
        ch = clean_channel_link(ch)
        try:
            await app.join_chat(ch)
            log.info(f"{tag} Join channel/group: {ch}")
        except Exception as e:
            # Udah member, invalid link, atau private channel yang butuh approval -> skip aja
            log.warning(f"{tag} Gagal/skip join {ch}: {e}")
        await rand_delay(DELAY_BETWEEN_STEPS)


# ==================== MAIN FLOW ====================
async def run_account(session_string, data, channels, index, total):
    ...
    try:
        ...
        # 0. Join channel/group dari channels.txt
        if channels:
            await join_channels(app, channels, tag)
...
async def main():
    ...
    try:
        ...
        channels = []
        log.warning(f"{CHANNELS_FILE} tidak ditemukan, skip join channel/group.")
    ...
    for i in range(start_idx, end_idx + 1):
        await run_account(sessions[i], data_rows[i], channels, i + 1, total)
        if i < end_idx:
