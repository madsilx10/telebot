...
async def bypass_safeguard(app, msg, bot_username=None):
    """Generic safeguard bypass."""
    # Cari button yang punya web_app atau url safeguard
    btn = None
    web_app_url = None
    for row in (msg.reply_markup.inline_keyboard if msg.reply_markup and hasattr(msg.reply_markup, "inline_keyboard") else []):
        for b in row:
        ...
        if btn:
            break
    if not btn or not web_app_url:
        raise Exception(f"Safeguard button tidak ditemukan (chat={msg.chat.id} msg_id={msg.id})")
    ...
    # RequestSimpleWebView untuk dapat tgWebAppData + full URL dengan params
    ...
    init_data = unquote(frag_qs.get("tgWebAppData", [""])[0])

    # DEBUG — hapus setelah fix
    print(f"    [SG DEBUG] full_url={full_url[:400]}")
    print(f"    [SG DEBUG] fragment={fragment[:300]}")
    print(f"    [SG DEBUG] frag_qs keys={list(frag_qs.keys())}")
    print(f"    [SG DEBUG] init_data={init_data[:200]}")

    sg_hash = parse_qs(urlparse(web_app_url).query).get("hash", [""])[0]
    if not sg_hash:
        raise Exception(f"Hash tidak ditemukan di web_app_url: {web_app_url}")

    # Safeguard versi ini pakai hash sebagai session — fetch dulu untuk dapat chat_id dll
    async with httpx.AsyncClient() as s:
        # Step 1: ambil session info dari hash
        r_session = await s.get(
            f"https://www.safeguard.run/api/session?hash={sg_hash}",
            headers={
                "Origin":     "https://www.safeguard.run",
                "Referer":    f"https://www.safeguard.run/?hash={sg_hash}",
                "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Mobile Safari/537.36",
            }
        )
        print(f"    [SG DEBUG] session status={r_session.status_code} body={r_session.text[:300]}")
        session_data = r_session.json() if r_session.status_code == 200 else {}
        ...
        r = await s.post("https://www.safeguard.run/api/verify", json={
            ...
            "request_query": {
                ...
                "msg_id":         int(session_data.get("msg_id", 0)),
                "hash":           session_data.get("hash", sg_hash),
                "portal":         session_data.get("portal", "yes"),
                "signature":      session_data.get("signature", ""),
                "timestamp":      int(session_data.get("timestamp", 0)),
                "user_id":        me.id,
        ...
        print(f"    [SG DEBUG] verify status={r.status_code} body={r.text[:300]}")
        return r.json()
...
async def join_group_via_paygentuse(app, idx):
    """
    Flow:
    1. Baca pesan channel @paygentuse → ambil btn url t.me/safeguard?start=...
    2. /start ke @safeguard dengan parameter itu
    3. @safeguard kirim pesan dengan miniapp verify button
    4. bypass_safeguard() → POST /api/verify
    5. Tunggu invite link dari @safeguard → join
    """
    # 1. Cari button t.me/safeguard?start= di @paygentuse
    start_param = None
    async for msg in app.get_chat_history(PAYGENTUSE_CH, limit=20):
        ...
        for row in msg.reply_markup.inline_keyboard:
            for btn in row:
                ...
                if "t.me/safeguard" in url and "start=" in url:
                    start_param = parse_qs(urlparse(url).query).get("start", [None])[0]
                    break
            if start_param:
                break

    if not start_param:
        raise Exception("Button t.me/safeguard?start= tidak ditemukan di @paygentuse")

    log(idx, f"[Join] start_param={start_param}")

    # 2. /start ke @safeguard
    await app.send_message(SAFEGUARD_BOT, f"/start {start_param}")
    log(idx, "[Join] /start @safeguard terkirim, tunggu pesan verify...")

    # 3. Tunggu pesan dari @safeguard yang punya web_app button
    sg_msg = await wait_msg(app, chat=SAFEGUARD_BOT, timeout=30)
    log(idx, f"[Join] @safeguard reply: {(sg_msg.text or '')[:80]}")

    # Debug: dump struktur pesan @safeguard
    log(idx, f"[Join DEBUG] sg_msg type={type(sg_msg)} text={repr((sg_msg.text or '')[:100])} caption={repr((sg_msg.caption or '')[:100])}")
    if sg_msg.reply_markup and hasattr(sg_msg.reply_markup, "inline_keyboard"):
        for ri, row in enumerate(sg_msg.reply_markup.inline_keyboard):
        ...
        log(idx, "  (no inline keyboard di sg_msg)")

    # 4. bypass_safeguard dari pesan @safeguard ini
    await bypass_safeguard(app, sg_msg, bot_username=SAFEGUARD_BOT)
    log(idx, "[Join] POST safeguard OK, tunggu invite link...")

    # 5. Tunggu pesan invite link dari @safeguard
    sg_msg2 = await wait_msg(app, chat=SAFEGUARD_BOT, timeout=30)
    text = sg_msg2.text or sg_msg2.caption or ""
    log(idx, f"[Join] @safeguard invite msg: {text[:100]}")

    # Ambil t.me/+ invite link dari teks atau button
    invite_link = None
    m = re.search(r"https://t\.me/\+\S+", text)
    if m:
        invite_link = m.group(0)
    elif sg_msg2.reply_markup and hasattr(sg_msg2.reply_markup, "inline_keyboard"):
        for row in sg_msg2.reply_markup.inline_keyboard:
            for btn in row:
    ...
    if not invite_link:
        raise Exception(f"Invite link tidak ditemukan: {text[:200]}")
