...
async def click_button(msg: Message, keyword: str):
...
async def run_account(idx: int, token: str, channels: list):
    ...
    async with Client(
        ...
        async for msg in app.get_chat_history(BOT_USERNAME, limit=5):
            if msg.reply_markup:
                if await click_button(msg, "check"):
                    await asyncio.sleep(DELAY)
                    break
        ...
        async for msg in app.get_chat_history(BOT_USERNAME, limit=5):
            if msg.reply_markup:
                clicked = await click_button(msg, "gemx box")
                if not clicked:
                    clicked = await click_button(msg, "box")
                if clicked:
                    await asyncio.sleep(DELAY)
                    break
