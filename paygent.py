...
async def bypass_safeguard(app, msg, bot_username=None):
    ...
    url = btn.web_app.url if hasattr(btn, "web_app") and btn.web_app else btn.url
    ...
    # peer harus chat yang bisa di-write (diri sendiri / "me")
    # channel read-only → 403 CHAT_WRITE_FORBIDDEN
    try:
        write_peer = await app.resolve_peer(msg.chat.id)
        # test apakah ini channel (tidak bisa write)
        if hasattr(write_peer, "channel_id"):
            write_peer = await app.resolve_peer("me")
    except Exception:
        write_peer = await app.resolve_peer("me")
