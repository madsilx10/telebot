...
async def bypass_safeguard(app, msg, bot_username=None):
    ...
    # Parse params dari tgWebAppStartParam atau dari URL asli
    # safeguard kadang taruh chat_id dll di query string web_app_url langsung
    p = parse_qs(urlparse(web_app_url).query)

    # Kalau query kosong, coba dari full_url query
    if not p.get("chat_id"):
        p = parse_qs(urlparse(full_url).query)

    log_tag = f"chat={msg.chat.id}"
    if not p.get("chat_id"):
        raise Exception(f"chat_id tidak ditemukan di URL safeguard: {full_url[:200]}")
