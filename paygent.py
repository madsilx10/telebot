...
async def bypass_safeguard(app, msg, bot_username=None):
    ...
    # Parse params — safeguard taruh di tgWebAppStartParam dalam init_data
    # decode init_data dulu untuk ambil start_param
    p = {}
    try:
        # init_data format: key=val&key=val (url-encoded)
        id_params = parse_qs(init_data)
        start_param_raw = id_params.get("start_param", [""])[0]
        if start_param_raw:
            # start_param biasanya: chat_id=-xxx&msg_id=xxx&hash=xxx&...
            p = parse_qs(unquote(start_param_raw))
    except Exception:
        pass

    # fallback: coba dari query string web_app_url / full_url
    if not p.get("chat_id"):
        p = parse_qs(urlparse(web_app_url).query)
    if not p.get("chat_id"):
        p = parse_qs(urlparse(full_url).query)

    log_tag = f"chat={msg.chat.id}"
    if not p.get("chat_id"):
        # dump init_data buat debug
        raise Exception(f"chat_id tidak ditemukan. init_data={init_data[:300]}")
