...
async def run_account(session_string, twitter_username, wallet_address, index):
    ...
    async with Client(
                ...
                for row in msg.reply_markup.inline_keyboard:
                    for btn in row:
                        ...
                        if any(k in label for k in ['continue', 'start', 'mulai', 'lanjut']):
                            ...
                            await asyncio.sleep(6)  # tunggu bot balas captcha
                            break
            break

        if not clicked_continue:
            print(f"[Akun {index}] ⚠️ Tombol Continue tidak ditemukan, coba lanjut...")
        ...
        for attempt in range(max_attempts):
            ...
            print(f"[Akun {index}] Attempt {attempt+1}/{max_attempts}...")
