    ...
    async with Client(
        ...
        async for msg in app.get_chat_history(BOT_USERNAME, limit=5):
            if msg.reply_markup:
                ...
                if clicked:
                    await asyncio.sleep(DELAY)
                    # Ambil & print balasan bot setelah klik
                    async for reply in app.get_chat_history(BOT_USERNAME, limit=3):
                        if reply.from_user and reply.from_user.username and                            reply.from_user.username.lower() == BOT_USERNAME.lower():
