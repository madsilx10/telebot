...
async def run_account(session_string: str, email: str, x_profile: str, wallet: str, index: int):
    ...
    async with client:
        ...
        try:
            ...
            # 2. Join Airdrop & Register (inline button)
            ...
            # 5. Registration (inline button, dari msg terakhir)
...
async def main():
    ...
    print(f"Pilihan: nomor akun (1-{total}) | 'all' | 'X-end' (misal: 3-end)")
    pilihan = input("Pilih: ").strip().lower()

    if pilihan == "all":
        indices = list(range(total))
    elif re.match(r"^\d+-end$", pilihan):
        start = int(pilihan.split("-")[0]) - 1
